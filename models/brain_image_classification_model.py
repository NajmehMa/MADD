...
class BrainImageClassificationModel():
    ...
    def train_function(self, data, model, optimizer, loss_fn,grad_scaler, device):
    ...
    def eval_function(self,data, model, device,is_inference=False):
        ...
        if is_inference:
            return {'scores': [np.max(x) for x in scores],
                    'preds': [self.class_labels[np.argmax(x)] for x in scores]}
        else:
    ...
    def train(self, load_model=False):
        ...
        for epoch in range(self.epochs):
            ...
            train_loss = self.train_function(train_loader, self.model, optimizer, loss_fn,grad_scaler, DEVICE)
    ...
    def test(self):
        ...
        eval_results = self.eval_function(test_loader, self.model, DEVICE)
    ...
    def input_dataset(self, dataset):
        ...
        self.model = resnet18(num_classes=self.n_classes)
